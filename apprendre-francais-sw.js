/* Apprendre le français — offline service worker
...
*/
const CACHE_PREFIX = "apprendre-francais-";
const CACHE_VERSION = "v1";
const APP_CACHE = `${CACHE_PREFIX}${CACHE_VERSION}`;

const APP_FILES = [
  "./apprendre-francais.html",
  "./apprendre-francais-manifest.json",
  "./apprendre-francais-icon-180.png",
  "./apprendre-francais-icon-192.png",
  "./apprendre-francais-icon-512.png"
];
...
async function networkFirstAppPage(request) {
  ...
  try {
    ...
    if (response.ok) {
      try {
        await cache.put("./apprendre-francais.html", response.clone());
      } catch (error) {
    ...
    return (
      (await cache.match("./apprendre-francais.html")) ||
      new Response(
        "<!doctype html><meta charset='utf-8'><title>Français offline</title>" +
