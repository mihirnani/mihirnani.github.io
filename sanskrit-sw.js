/* Saṃskṛtam — offline service worker
   Page-specific service worker for /sanskrit.html.
   Online visits use the newest HTML; offline visits use the last cached copy.
*/

"use strict";

const CACHE_PREFIX = "sanskrit-";
const CACHE_VERSION = "v1";
const CACHE_NAME = `${CACHE_PREFIX}${CACHE_VERSION}`;

const APP_PAGE = "/sanskrit.html";

const APP_ASSETS = [
  APP_PAGE,
  "/sanskrit-manifest.json",
  "/sanskrit-icon-180.png",
  "/sanskrit-icon-192.png",
  "/sanskrit-icon-512.png"
];
...
self.addEventListener("activate", event => {
  event.waitUntil(
    (async () => {
      ...
      /*
        Delete only older Saṃskṛtam caches.
