...
SW_SCRIPT = """<script>if ('serviceWorker' in navigator) {
  window.addEventListener('load', function () {
    navigator.serviceWorker.register('%s', {scope:'./'}).catch(function(error){
...
def tail(coll):
    ...
    if coll.get("sw"):
        parts.append(SW_SCRIPT % coll["sw"])
    return "\n".join(parts) + "</body></html>\n"

CSS_BANNER = ("/* Generated by tools/shell.py from assets/collection.css - do not edit this copy.\n"
              "   Each collection keeps its own copy so that its offline service worker can cache it. */\n")
