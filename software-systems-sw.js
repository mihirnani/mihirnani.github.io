/* Software Systems Engineering — offline service worker
...
*/
const CACHE_PREFIX = "software-systems-";
const CACHE_VERSION = "v1";
const APP_CACHE = `${CACHE_PREFIX}${CACHE_VERSION}`;

const APP_FILES = [
  "./software-systems.html",
  "./software-systems-manifest.json",
  "./software-systems-icon-180.png",
  "./software-systems-icon-192.png",
  "./software-systems-icon-512.png"
];
...
self.addEventListener("install", (event) => {
  event.waitUntil((async () => {
    ...
    // One missing optional asset must not prevent installation.
...
async function networkFirstAppPage(request) {
  ...
  try {
    ...
    if (response.ok) {
      try {
        await cache.put("./software-systems.html", response.clone());
      } catch (error) {
        // A cache write failure must not block a successful online load.
      }
    }
    ...
    return (
      (await cache.match("./software-systems.html")) ||
      new Response(
        "<!doctype html><meta charset='utf-8'><title>Software Systems offline</title>" +
        "<main style='font:16px system-ui;padding:2rem;max-width:40rem'>" +
        "<h1>Software Systems Engineering</h1>" +
  ...
  try {
    ...
    if (response.ok) {
      try {
        ...
        // Serve the asset even if it cannot be cached.
      }
    }
